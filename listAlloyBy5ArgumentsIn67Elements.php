...
$minAlpha     = isset($_POST['minAlpha']) ? $_POST['minAlpha'] : '';
$maxAlpha     = isset($_POST['maxAlpha']) ? $_POST['maxAlpha'] : '';
$postElements = isset($_POST['elements']) ? $_POST['elements'] : '';

if ($number!='' && $minSigma!='' && $maxSigma!='' && $minDeltaH!='' && $maxDeltaH!='' && $minGma!='' && $maxGma!=''&& $minOmega!='' && $maxOmega!=''&& $minAlpha!='' && $maxAlpha!='') {
                                                ...
                                                for ($i3=$i2+1; $i3<68; $i3++) {
                                                        for ($i4=$i3+1; $i4<68; $i4++) {
                                                                for ($i5=$i4+1; $i5<68; $i5++) {
                                                                        ...
                                                                        $alpha     = getAlpha($elements, $numbers, $radium);
                                                                        
                                                                        if ( $sigma >= $minSigma && $sigma <= $maxSigma && $deltaHmix >= $minDeltaH && $deltaHmix <= $maxDeltaH && $gma >= $minGma && $gma <= $maxGma && $omega >= $minOmega && $omega <= $maxOmega && $alpha >= $minAlpha && $alpha <= $maxAlpha) {
                                                                                echo $elementsMapping[$i1]."  ".$elementsMapping[$i2]."  ".$elementsMapping[$i3]."  ".$elementsMapping[$i4]."  ".$elementsMapping[$i5]."    -----    Sigma: $sigma    -----    deltaHmix: $deltaHmix    -----    Gma: $gma    -----    Omega: $omega    -----    Alpha2: $alpha"."<br/>";
                                                                        }
                                                                }
                                                        }
                                                }
                                                        ...
                                                        for ($i4=$i3+1; $i4<68; $i4++) {
                                                                for ($i5=$i4+1; $i5<68; $i5++) {
                                                                        for ($i6=$i5+1; $i6<68; $i6++) {
                                                                                ...
                                                                                if ( $sigma >= $minSigma && $sigma <= $maxSigma && $deltaHmix >= $minDeltaH && $deltaHmix <= $maxDeltaH && $gma >= $minGma && $gma <= $maxGma && $omega >= $minOmega && $omega <= $maxOmega && $alpha >= $minAlpha && $alpha <= $maxAlpha) {
                                                                                        echo $elementsMapping[$i1]."  ".$elementsMapping[$i2]."  ".$elementsMapping[$i3]."  ".$elementsMapping[$i4]."  ".$elementsMapping[$i5]."  ".$elementsMapping[$i6]."    -----    Sigma: $sigma    -----    deltaHmix: $deltaHmix    -----    Gma: $gma    -----    Omega: $omega    -----    Alpha2: $alpha"."<br/>";
                                                                                }
                                                                        }
                                                                }
                                                        }
                                                                ...
                                                                for ($i5=$i4+1; $i5<68; $i5++) {
                                                                        for ($i6=$i5+1; $i6<68; $i6++) {
                                                                                for ($i7=$i6+1; $i7<68; $i7++) {
                                                                                        ...
                                                                                        if ( $sigma >= $minSigma && $sigma <= $maxSigma && $deltaHmix >= $minDeltaH && $deltaHmix <= $maxDeltaH && $gma >= $minGma && $gma <= $maxGma  && $omega >= $minOmega && $omega <= $maxOmega && $alpha >= $minAlpha && $alpha <= $maxAlpha) {
                                                                                                echo $elementsMapping[$i1]."  ".$elementsMapping[$i2]."  ".$elementsMapping[$i3]."  ".$elementsMapping[$i4]."  ".$elementsMapping[$i5]."  ".$elementsMapping[$i6]."  ".$elementsMapping[$i7]."    -----    Sigma: $sigma    -----    deltaHmix: $deltaHmix    -----    Gma: $gma    -----    Omega: $omega    -----    Alpha2: $alpha"."<br/>";
                                                                                        }
                                                                                }
                                                                        }
                                                                }
                                                                        ...
                                                                        for ($i6=$i5+1; $i6<68; $i6++) {
                                                                                for ($i7=$i6+1; $i7<68; $i7++) {
                                                                                        for ($i8=$i7+1; $i8<68; $i8++) {
                                                                                                ...
                                                                                                if ( $sigma >= $minSigma && $sigma <= $maxSigma && $deltaHmix >= $minDeltaH && $deltaHmix <= $maxDeltaH && $gma >= $minGma && $gma <= $maxGma  && $omega >= $minOmega && $omega <= $maxOmega && $alpha >= $minAlpha && $alpha <= $maxAlpha) {
                                                                                                        echo $elementsMapping[$i1]."  ".$elementsMapping[$i2]."  ".$elementsMapping[$i3]."  ".$elementsMapping[$i4]."  ".$elementsMapping[$i5]."  ".$elementsMapping[$i6]."  ".$elementsMapping[$i7]."  ".$elementsMapping[$i8]."    -----    Sigma: $sigma    -----    deltaHmix: $deltaHmix    -----    Gma: $gma    -----    Omega: $omega    -----    Alpha2: $alpha"."<br/>";
                                                                                                }
                                                                                        }
                                                                                }
                                                                        }
                                                ...
                                                for ($i3=$i2+1; $i3<$countPostElements; $i3++) {
                                                        for ($i4=$i3+1; $i4<$countPostElements; $i4++) {
                                                                for ($i5=$i4+1; $i5<$countPostElements; $i5++) {
                                                                        ...
                                                                        $alpha     = getAlpha($elements, $numbers, $radium_plus);
                                                                        
                                                                        if ( $sigma >= $minSigma && $sigma <= $maxSigma && $deltaHmix >= $minDeltaH && $deltaHmix <= $maxDeltaH && $gma >= $minGma && $gma <= $maxGma && $omega >= $minOmega && $omega <= $maxOmega && $alpha >= $minAlpha && $alpha <= $maxAlpha) {
                                                                                echo $elementsMapping_plus[$i1]."  ".$elementsMapping_plus[$i2]."  ".$elementsMapping_plus[$i3]."  ".$elementsMapping_plus[$i4]."  ".$elementsMapping_plus[$i5]."    -----    Sigma: ". sprintf("%.8f", $sigma)."    -----    deltaHmix: $deltaHmix    -----    Gma: $gma    -----    Omega: $omega    -----    Alpha2: $alpha"."<br/>";
                                                                        }
                                                                }
                                                        }
                                                }
                                                        ...
                                                        for ($i4=$i3+1; $i4<$countPostElements; $i4++) {
                                                                for ($i5=$i4+1; $i5<$countPostElements; $i5++) {
                                                                        for ($i6=$i5+1; $i6<$countPostElements; $i6++) {
                                                                                ...
                                                                                if ( $sigma >= $minSigma && $sigma <= $maxSigma && $deltaHmix >= $minDeltaH && $deltaHmix <= $maxDeltaH && $gma >= $minGma && $gma <= $maxGma  && $omega >= $minOmega && $omega <= $maxOmega && $alpha >= $minAlpha && $alpha <= $maxAlpha) {
                                                                                        echo $elementsMapping_plus[$i1]."  ".$elementsMapping_plus[$i2]."  ".$elementsMapping_plus[$i3]."  ".$elementsMapping_plus[$i4]."  ".$elementsMapping_plus[$i5]."  ".$elementsMapping_plus[$i6]."    -----    Sigma: ". sprintf("%.8f", $sigma)."    -----    deltaHmix: $deltaHmix    -----    Gma: $gma    -----    Omega: $omega    -----    Alpha2: $alpha"."<br/>";
                                                                                }
                                                                        }
                                                                }
                                                        }
                                                                ...
                                                                for ($i5=$i4+1; $i5<$countPostElements; $i5++) {
                                                                        for ($i6=$i5+1; $i6<$countPostElements; $i6++) {
                                                                                for ($i7=$i6+1; $i7<$countPostElements; $i7++) {
                                                                                        ...
                                                                                        if ( $sigma >= $minSigma && $sigma <= $maxSigma && $deltaHmix >= $minDeltaH && $deltaHmix <= $maxDeltaH && $gma >= $minGma && $gma <= $maxGma  && $omega >= $minOmega && $omega <= $maxOmega && $alpha >= $minAlpha && $alpha <= $maxAlpha) {
                                                                                                echo $elementsMapping_plus[$i1]."  ".$elementsMapping_plus[$i2]."  ".$elementsMapping_plus[$i3]."  ".$elementsMapping_plus[$i4]."  ".$elementsMapping_plus[$i5]."  ".$elementsMapping_plus[$i6]."  ".$elementsMapping_plus[$i7]."    -----    Sigma: ". sprintf("%.8f", $sigma)."    -----    deltaHmix: $deltaHmix    -----    Gma: $gma    -----    Omega: $omega    -----    Alpha2: $alpha"."<br/>";
                                                                                        }
                                                                                }
                                                                        }
                                                                }
                                                                        ...
                                                                        for ($i6=$i5+1; $i6<$countPostElements; $i6++) {
                                                                                for ($i7=$i6+1; $i7<$countPostElements; $i7++) {
                                                                                        for ($i8=$i7+1; $i8<$countPostElements; $i8++) {
                                                                                                ...
                                                                                                if ( $sigma >= $minSigma && $sigma <= $maxSigma && $deltaHmix >= $minDeltaH && $deltaHmix <= $maxDeltaH && $gma >= $minGma && $gma <= $maxGma  && $omega >= $minOmega && $omega <= $maxOmega && $alpha >= $minAlpha && $alpha <= $maxAlpha) {
                                                                                                        echo $elementsMapping_plus[$i1]."  ".$elementsMapping_plus[$i2]."  ".$elementsMapping_plus[$i3]."  ".$elementsMapping_plus[$i4]."  ".$elementsMapping_plus[$i5]."  ".$elementsMapping_plus[$i6]."  ".$elementsMapping_plus[$i7]."  ".$elementsMapping_plus[$i8]."    -----    Sigma: ". sprintf("%.8f", $sigma)."    -----    deltaHmix: $deltaHmix    -----    Gma: $gma    -----    Omega: $omega    -----    Alpha2: $alpha"."<br/>";
                                                                                                }
                                                                                        }
                                                                                }
                                                                        }
 ...
 * @date 2015-3-7
 */
function getAlpha($elements, $numbers, $radium) {
    // 如果元素数组与原子个数数组的元素数量不等，返回-1
    if (count($elements) != count($numbers)) {
        return -1;
    }
    ...
    for($i=0; $i<count($elements); $i++) {
        for ($j=$i; $j<count($elements); $j++) {
            $alpha += ($C[$i] * $C[$j] * abs($radium[$elements[$i]] + $radium[$elements[$j]] - $doubleAverageR))/$doubleAverageR;
        }
    }
...
<html>
    <body>
        ...
        <form name="input" method="post" >
            ...
            The the range of Alpha2:<br/>
            <input type="text" name="minAlpha" style="width:200px;height:30px">
            ~
            <input type="text" name="maxAlpha" style="width:200px;height:30px"><br/><br/>
